...
enum class RepeatType(val label: String) {
    None("无"), Daily("每天"), Weekly("每周"), Monthly("每月"), Yearly("每年")
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ToDoAlertDialog(
    viewModel: INoteViewModel, showDialog: MutableState<Boolean>, onDismiss: () -> Unit
) {
    ...
    if (showDialog.value) {
        ...
        Dialog(onDismissRequest = onDismiss) {
            Box(
                ...
                Surface(
                    shape = MaterialTheme.shapes.large, tonalElevation = 16.dp, modifier = Modifier
                ) {
                    Column(
